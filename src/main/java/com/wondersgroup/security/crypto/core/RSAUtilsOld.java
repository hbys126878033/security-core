...
public class RSAUtilsOld {
	...
	public static String getPublicExponent() {
		BigInteger b = ((BCRSAPublicKey) KEY_PAIR.getPublic()).getPublicExponent();
	...
	public static String getModulus() {
		BigInteger b = ( (BCRSAPublicKey)KEY_PAIR.getPublic()).getModulus();
		return b.toString(16);
	}

	public static String getPublicKey(){
		BCRSAPublicKey aPublic = (BCRSAPublicKey)KEY_PAIR.getPublic();
	...
	public static String getPrivateExponent() {
		BigInteger b = ((BCRSAPrivateKey) KEY_PAIR.getPrivate()).getPrivateExponent();
		return b.toString(16);
	}
	...
	public static void main(String[] args) {
		try {
			...
			BigInteger publicModulus = aPublic.getModulus();
			BigInteger publicExponent = aPublic.getPublicExponent();
			System.out.println("publicModulus 模：\n"+publicModulus.toString());
			System.out.println("publicExponent 指数：\n"+publicExponent.toString());

			RSAPublicKeySpec rsaPublicKeySpec = new RSAPublicKeySpec(publicModulus, publicExponent);
			PublicKey rsa = KeyFactory.getInstance("RSA").generatePublic(rsaPublicKeySpec);
			System.out.println(rsa.getClass());
			String publicKeyString2 = Base64.getEncoder().encodeToString(rsa.getEncoded());
			System.out.println("根据模和指数，还原公钥\n"+publicKeyString2);
			System.out.println("公钥是否相等:" +(publicKeyString2.equals(publicKeyString1)));

			BCRSAPrivateCrtKey aPrivate = (BCRSAPrivateCrtKey) KEY_PAIR.getPrivate();
			/* org.bouncycastle.jcajce.provider.asymmetric.rsa.BCRSAPrivateCrtKey */
			System.out.println(aPrivate.getClass());

			String privateKeyString1 = Base64.getEncoder().encodeToString(aPrivate.getEncoded());
			System.out.println("RSA 私钥 :\n"+ privateKeyString1);

			BigInteger privateModulus = aPrivate.getModulus();
			BigInteger privateExponent = aPrivate.getPrivateExponent();
			System.out.println("privateModulus 模：\n"+privateModulus.toString());
			System.out.println("privateExponent 指数：\n"+privateExponent.toString());
			...
			RSAPrivateCrtKeySpec rsaPrivateCrtKeySpec = new RSAPrivateCrtKeySpec(
					...
					aPrivate.getPrimeP(),
					aPrivate.getPrimeQ(),
					aPrivate.getPrimeExponentP(),
					aPrivate.getPrimeExponentQ(),
					aPrivate.getCrtCoefficient());

			PrivateKey rsa1 = KeyFactory.getInstance("RSA").generatePrivate(rsaPrivateCrtKeySpec);
			String privateKeyString2 = Base64.getEncoder().encodeToString(rsa1.getEncoded());
			System.out.println("RSA 私钥2:\n"+privateKeyString2);
			System.out.println("私钥是否相当====>"+(privateKeyString2.equals(privateKeyString1)));
